...
function sanitizePublicText(value) {
  return String(value ?? "")
    ...
    .replace(/\bNot Implemented In This Sprint\b/gi, "Capability Boundary")
    .replace(/\bNot Implemented\b/gi, "Capability Boundary")
    .replace(/\bRecommended Next UI Sprint\b/gi, "Operator Planning Notes")
    .replace(/\bRecommended Next Sprint\b/gi, "Operator Planning Notes")
    .replace(/\bRemaining Blockers\b/gi, "Operator Conditions")
    .replace(/\bRemaining blocker\b/gi, "Operator condition")
    .replace(/\bprototype\b/gi, "controlled validation")
    .replace(/\bexperimental\b/gi, "validation")
    .replace(/\bGuided Preview Access\b/g, "Secure Workspace Access")
    .replace(/\bguided preview access\b/g, "secure workspace access")
    .replace(/\bGuided Preview\b/g, "Secure Workspace")
    .replace(/\bguided preview\b/g, "secure workspace")
    .replace(/\bDemo Role Switcher\b/g, "Workspace View")
    .replace(/\bdemo role switcher\b/g, "workspace view")
    .replace(/\bDEMO MODE\b/g, "SECURE WORKSPACE ACCESS")
    .replace(/\bDemo Mode\b/g, "Secure Workspace Access")
    .replace(/\bdemo mode\b/g, "secure workspace access")
    .replace(/\bDEMO DATA(?:\s*(?:--|—)\s*NOT REAL PATIENT DATA)?\b/g, "PROTECTED WORKSPACE RECORDS")
    .replace(/\bDemo Data(?:\s*(?:--|—)\s*Not Real Patient Data)?\b/g, "Protected Workspace Records")
    .replace(/\bdemo data(?:\s*(?:--|—)\s*not real patient data)?\b/g, "protected workspace records")
    .replace(/\bNo Real Patient Data\b/g, "Protected Workspace Records")
    .replace(/\bno real patient data\b/g, "protected workspace records")
    .replace(/\bnot real patient data\b/gi, "protected workspace records")
    .replace(/\bNo real patient data used\b/g, "Protected workspace record boundary confirmed")
    ...
    .replace(/\bcontrolled pilot\b/gi, "governed validation")
    .replace(/\bexternal pilot\b/gi, "external deployment validation")
    .replace(/\bPilot\b/g, "Enterprise Validation")
    .replace(/\bpilot\b/g, "enterprise validation")
    .replace(/\bguided validation\b/gi, "governed validation")
    .replace(/\bSecure Preview Access\b/g, "Secure Workspace Access")
    .replace(/\bsecure preview access\b/g, "secure workspace access")
    .replace(/\bSecure Preview\b/g, "Secure Workspace")
    .replace(/\bsecure preview\b/g, "secure workspace")
    .replace(/\bProtected Sample Records\b/g, "Protected Workspace Records")
    .replace(/\bprotected sample records\b/g, "protected workspace records")
    .replace(/\bSample Records\b/g, "Workspace Records")
    .replace(/\bsample records\b/g, "workspace records")
    .replace(/\bsample data\b/gi, "workspace records")
    .replace(/\bsample provider\b/gi, "reference provider")
    .replace(/\bsample\b/gi, "reference")
    .replace(/\bsynthetic records\b/gi, "protected workspace records")
    .replace(/\bsynthetic\b/gi, "protected workspace")
