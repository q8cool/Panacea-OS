...
import { API_BASE_PATH, SERVICE_NAME } from "../domain/privacy-domain.mjs";
import {
  PrivacyAuthorizationError,
  PrivacyValidationError
} from "../domain/privacy-validation.mjs";
import { createHeaderPrivacyAuthenticator } from "../infrastructure/security.mjs";
import { buildOpenApiDocument } from "./openapi.mjs";
import { routeByFullPath } from "./routes.mjs";

const groupMethodName = {
  global_consent: "recordGlobalConsent",
  patient_data_rights: "recordPatientDataRights",
  privacy_policy_engine: "recordPrivacyPolicyEngine",
  data_sharing_governance: "recordDataSharingGovernance",
  trust_platform: "recordTrustPlatform",
  privacy_monitoring: "recordPrivacyMonitoring"
};
...
function toErrorResponse(error) {
  if (error instanceof PrivacyAuthorizationError) {
    return {
  ...
  if (error instanceof PrivacyValidationError || error instanceof SyntaxError) {
    return {
      statusCode: 400,
      body: {
  ...
  return {
    ...
    body: {
      ...
      message: "The global privacy platform could not process the request."
    }
  };
}

export function createGlobalPrivacyRequestHandler({
  service,
  authenticator = createHeaderPrivacyAuthenticator(),
  openApiDocument = buildOpenApiDocument()
}) {
  return async function handleGlobalPrivacyRequest(request, response) {
    try {
      ...
      if (request.method === "GET" && url.pathname === `${API_BASE_PATH}/metrics`) {
        return jsonResponse(response, 200, {
          ...
          consentEnforcement: "required",
          purposeBasedAccess: "required",
          crossBorderPolicy: "enforced",
      ...
      if (!url.pathname.startsWith(API_BASE_PATH)) {
        return jsonResponse(response, 404, { error: "not_found", message: "Route is outside the global privacy API." });
      }
      ...
      if (!route || request.method !== route.method) {
        return jsonResponse(response, 404, { error: "not_found", message: "Global privacy route not found." });
      }
      const principal = authenticator.authenticate(request);
      const body = await readJson(request);
...
export function createGlobalPrivacyServer(options) {
  return http.createServer(createGlobalPrivacyRequestHandler(options));
}
