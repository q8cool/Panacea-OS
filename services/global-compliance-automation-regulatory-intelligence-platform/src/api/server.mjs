...
import { API_BASE_PATH, SERVICE_NAME } from "../domain/compliance-domain.mjs";
import {
  ComplianceAuthorizationError,
  ComplianceValidationError
} from "../domain/compliance-validation.mjs";
import { createHeaderComplianceAuthenticator } from "../infrastructure/security.mjs";
import { buildOpenApiDocument } from "./openapi.mjs";
import { routeByFullPath } from "./routes.mjs";

const groupMethodName = {
  regulatory_intelligence: "recordRegulatoryIntelligence",
  compliance_automation: "recordComplianceAutomation",
  audit_management: "recordAuditManagement",
  certification_management: "recordCertificationManagement",
  policy_compliance: "recordPolicyCompliance",
...
function toErrorResponse(error) {
  if (error instanceof ComplianceAuthorizationError) {
    return {
  ...
  if (error instanceof ComplianceValidationError || error instanceof SyntaxError) {
    return {
      statusCode: 400,
      body: {
  ...
  return {
    ...
    body: {
      ...
      message: "The global compliance automation platform could not process the request."
    }
  };
}

export function createGlobalComplianceRequestHandler({
  service,
  authenticator = createHeaderComplianceAuthenticator(),
    ...
    try {
      ...
      if (request.method === "GET" && url.pathname === `${API_BASE_PATH}/metrics`) {
        return jsonResponse(response, 200, {
          ...
          complianceAutomation: "policy_controlled",
      ...
      if (!url.pathname.startsWith(API_BASE_PATH)) {
        return jsonResponse(response, 404, { error: "not_found", message: "Route is outside the global compliance API." });
      }
      ...
      if (!route || request.method !== route.method) {
        return jsonResponse(response, 404, { error: "not_found", message: "Global compliance route not found." });
      }
      const principal = authenticator.authenticate(request);
      const body = await readJson(request);
...
export function createGlobalComplianceServer(options) {
  return http.createServer(createGlobalComplianceRequestHandler(options));
}
